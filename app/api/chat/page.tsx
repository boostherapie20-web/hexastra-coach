...
import { PaywallInline, DailyLimitReached, UpgradeBanner } from '@/components/paywall'
...
export default function ChatPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (profile) setUserPlan(profile.plan as Plan)

      // Compter les lectures du jour (plan gratuit)
      ...
      setReadingsToday(count || 0)
    })
  }, [])

  useEffect(() => { endRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [messages, isTyping])

  const handleUpgrade = async (priceKey = 'premium_monthly') => {
  ...
  const handleSend = useCallback(async (birthData?: { birthDateISO: string; lat: number; lon: number; city?: string }) => {
    ...
    // Vérification limite quotidienne (plan gratuit)
    if (userPlan === 'free' && birthData && readingsToday >= MAX_FREE_READINGS) {
    ...
    if (birthData) setReadingsToday(prev => prev + 1)
    ...
    try {
      ...
      setTimeout(() => {
        setIsTyping(false)
        const isFreeAnalysis = !!birthData && userPlan === 'free'
        setMessages(prev => [...prev, {
          id: Date.now().toString(), role: 'assistant',
          content: birthData
            ? isFreeAnalysis
              ? "Voici un aperçu de votre analyse.\n\nVotre profil révèle une nature profondément introspective, en quête de sens. Cette période de vie est marquée par une transition importante — une invitation à réévaluer vos priorités et à faire confiance à votre intuition.\n\nPour accéder à l'analyse complète..."
              : "Votre analyse complète est prête. Votre thème natal révèle une nature profonde, en quête de sens et d'authenticité. Les configurations planétaires actuelles indiquent une période favorable aux décisions importantes."
            : "Je vous entends. Pour une analyse personnalisée, cliquez sur « Démarrer une lecture ».",
          created_at: new Date().toISOString(),
          showPaywall: isFreeAnalysis,
        }])
      }, 3000)
    } catch {
      setIsTyping(false)
    }
  }, [input, conversationId, userPlan, readingsToday])
  ...
  const SidebarContent = () => (
    <>
      ...
      {/* Plan badge dans la sidebar */}
      <div style={s.planBadgeWrap}>
        <div style={{ ...s.planBadge, ...(userPlan === 'free' ? s.planBadgeFree : userPlan === 'premium' ? s.planBadgePremium : s.planBadgePraticien) }}>
          {userPlan === 'free' ? '⬡ Gratuit' : userPlan === 'premium' ? '⭐ Premium' : '◆ Praticien'}
        </div>
        {userPlan === 'free' && (
          <button onClick={() => handleUpgrade()} style={s.upgradeSmallBtn}>
            Passer Premium →
          </button>
        )}
      </div>
    ...
    <div style={s.root}>
      ...
      <div style={s.mobileHeader}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} style={s.hamburger}>
            {mobileMenuOpen
      ...
      {mobileMenuOpen && (
        <div style={s.mobileOverlay} onClick={() => setMobileMenuOpen(false)}>
          <div style={s.mobileMenu} onClick={e => e.stopPropagation()}><SidebarContent /></div>
        </div>
      )}
      ...
      <main style={s.main}>
        <header style={s.header}>
          ...
          <button onClick={() => {
            if (userPlan === 'free' && readingsToday >= MAX_FREE_READINGS) {
              setShowDailyLimit(true)
            } else {
        ...
        <div style={s.msgs}>
          {messages.map((msg, i) => (
            <div key={msg.id} style={{
              ...
              <div style={{ ...s.bub, ...(msg.role === 'user' ? s.bubU : s.bubAI) }}>
                ...
                {/* PAYWALL INLINE pour les utilisateurs gratuits */}
                {msg.showPaywall && (
                  <PaywallInline userPlan={userPlan} onUpgrade={() => handleUpgrade()} />
                )}
          ...
          {showDailyLimit && (
            <div style={{ padding: '0 0 8px', animation: 'fadeUp 0.35s cubic-bezier(0.16,1,0.3,1) both' }}>
              <DailyLimitReached onUpgrade={() => handleUpgrade()} />
            </div>
          )}
        ...
        {/* UPGRADE BANNER en bas */}
        <UpgradeBanner userPlan={userPlan} readingsToday={readingsToday} maxReadings={MAX_FREE_READINGS} />
        ...
        <div style={s.composer}>
          <div style={s.compInner}>
            <textarea
              ...
              rows={1} style={s.ta}
            />
            <button onClick={() => handleSend()} disabled={!input.trim() || isTyping} style={{ ...s.sendBtn, opacity: !input.trim() || isTyping ? 0.35 : 1 }}>
  ...
  return (
    <div style={m.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={m.modal}>
        <div style={m.hdr}>
          <div>
            <div style={m.stepLbl}>// Votre analyse commence ici</div>
        ...
        <div style={m.fields}>
          ...
          <div style={m.field}>
            ...
            <input type="text" placeholder="Ex : Paris, Lyon, Casablanca…" value={city} onChange={e => setCity(e.target.value)} style={m.input} autoComplete="off" />
...
const s: Record<string, React.CSSProperties> = {
  ...
  mobileHeader: { display: 'none', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, background: 'rgba(44,31,26,0.95)', backdropFilter: 'blur(16px)', borderBottom: '1px solid var(--b1)', padding: '10px 16px', alignItems: 'center', justifyContent: 'space-between' },
  hamburger: { width: '36px', height: '36px', background: 'rgba(255,255,255,0.06)', border: '1px solid var(--b2)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--tx2)', cursor: 'pointer' },
  ...
  mobileMenu: { position: 'absolute', left: 0, top: 0, bottom: 0, width: '240px', background: 'var(--pitch)', borderRight: '1px solid var(--b1)', display: 'flex', flexDirection: 'column' },
  ...
  navItem: { display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 10px', borderRadius: '6px', fontFamily: 'var(--f-ui)', fontSize: '12.5px', color: 'var(--tx3)', textAlign: 'left' as const, background: 'transparent', border: 'none', cursor: 'pointer', width: '100%' },
  ...
  planBadgeWrap: { padding: '12px 14px', display: 'flex', flexDirection: 'column' as const, gap: '8px', borderTop: '1px solid var(--b1)' },
  planBadge: { fontFamily: 'var(--f-mono)', fontSize: '9px', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '5px 10px', borderRadius: '4px', textAlign: 'center' as const },
  planBadgeFree: { background: 'rgba(255,255,255,0.04)', color: 'var(--tx3)', border: '1px solid var(--b2)' },
  planBadgePremium: { background: 'rgba(31,175,140,0.08)', color: 'var(--emerald)', border: '1px solid rgba(31,175,140,0.2)' },
  planBadgePraticien: { background: 'rgba(255,215,0,0.07)', color: '#ffd700', border: '1px solid rgba(255,215,0,0.2)' },
  upgradeSmallBtn: { fontFamily: 'var(--f-mono)', fontSize: '9.5px', letterSpacing: '0.1em', color: 'var(--emerald)', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'center' as const, textDecoration: 'underline' },
  ...
  newBtn: { background: 'rgba(31,175,140,0.08)', border: '1px solid rgba(31,175,140,0.25)', color: 'var(--emerald)', fontFamily: 'var(--f-mono)', fontSize: '10.5px', letterSpacing: '0.1em', padding: '8px 16px', borderRadius: '4px', whiteSpace: 'nowrap' as const, cursor: 'pointer' },
  ...
  sendBtn: { width: '32px', height: '32px', flexShrink: 0, background: 'var(--emerald)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--void)', border: 'none', cursor: 'pointer', boxShadow: '0 4px 14px rgba(31,175,140,0.3)' },
}
...
const m: Record<string, React.CSSProperties> = {
  ...
  input: { background: 'rgba(255,255,255,0.04)', border: '1px solid var(--b2)', borderRadius: '6px', padding: '10px 13px', color: 'var(--tx1)', fontSize: '13.5px', width: '100%', outline: 'none', fontFamily: 'var(--f-ui)' },
