  ...
  return (
    <aside
      style={{
        ...
        gap: 14,
        zIndex: 15,
        background: 'linear-gradient(180deg, rgba(10,7,5,0.54), rgba(10,7,5,0.76))',
    ...
    >
      <div style={{ display: 'flex', justifyContent: collapsed ? 'center' : 'space-between', alignItems: 'center', gap: 10 }}>
        {!collapsed && (
          <div>
        ...
        <IconBtn tooltip={collapsed ? 'Ouvrir le rail' : 'Réduire le rail'} onClick={() => onToggleCollapse?.()}>
      ...
      <Card hover={false} style={{ padding: collapsed ? 10 : 14, borderRadius: 24, flex: 1, overflow: 'hidden' }}>
        {!collapsed && <Label>Catégories</Label>}
        <div style={{ overflowY: 'auto', height: '100%', paddingRight: collapsed ? 0 : 2, display: 'flex', flexDirection: 'column', gap: collapsed ? 8 : 0, alignItems: collapsed ? 'center' : 'stretch' }}>
          {collapsed
            ? menu.map((item: any) => (
                <button
                  key={item.id}
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    setRightRailCollapsed(viewportWidth < 1640)
  }, [viewportWidth])
...
${content}`)
  ...
  const renderAssistantPreview = (
    <Card style={{ padding: 0, overflow: 'hidden', minHeight: 480, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '18px 20px', borderBottom: `1px solid ${DS.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div>
          <div style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(212,165,116,0.48)', fontFamily: DS.fontMono }}>Aperçu du chat</div>
  ...
  const rightRail = (
    <RightSidebar mode={mode} readings={readings} onSend={(t: string) => send(t)} onOpenReading={openReading} dragId={dragId} setDragId={setDragId} collapsed={rightRailCollapsed} onToggleCollapse={() => setRightRailCollapsed((v) => !v)} />
  )
  ...
  return (
    <div style={{ minHeight: '100vh', overflow: 'hidden', position: 'relative', background: DS.bg0 }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'grid', gridTemplateColumns: `${isDesktopLeft ? '272px ' : ''}minmax(0,1fr)${isDesktopRight ? (rightRailCollapsed ? ' 76px' : ' 248px') : ''}`, minHeight: '100vh' }}>
        ...
        <main style={{ minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
          <div style={{ padding: viewportWidth >= 1024 ? '20px 28px 14px' : '16px 16px 12px', position: 'sticky', top: 0, zIndex: 22, background: 'linear-gradient(180deg, rgba(9,7,5,0.82), rgba(9,7,5,0.42))', backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ maxWidth: 1220, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(212,165,116,0.52)', fontFamily: DS.fontMono, marginBottom: 4 }}>Conversation privée · orientation premium</div>
                  <div style={{ fontSize: viewportWidth >= 900 ? 24 : 20, color: DS.tx1, fontFamily: DS.fontTitle, fontWeight: 600, lineHeight: 1.1, letterSpacing: '-0.03em' }}>HexAstra <span style={{ color: DS.amber }}>Coach</span><span style={{ marginLeft: 10, display: viewportWidth >= 980 ? 'inline-flex' : 'none', verticalAlign: 'middle', padding: '4px 10px', borderRadius: 999, border: '1px solid rgba(212,165,116,0.14)', background: 'rgba(212,165,116,0.06)', fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(212,165,116,0.62)', fontFamily: DS.fontMono }}>Live</span></div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '0 12px', height: 38, borderRadius: 999, border: `1px solid ${DS.border}`, background: 'rgba(255,255,255,0.03)', color: DS.tx2, fontSize: 12, fontFamily: DS.fontBody }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: '#57d27c', boxShadow: '0 0 0 4px rgba(87,210,124,0.10)' }} />Réponse en quelques secondes</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: viewportWidth >= 1100 ? 'minmax(0, 1.04fr) minmax(360px, 0.86fr)' : '1fr', gap: 20, alignItems: 'stretch' }}>
                  <Card style={{ padding: viewportWidth >= 900 ? '42px 44px' : '28px 24px', borderRadius: 32, minHeight: viewportWidth >= 1100 ? 560 : undefined, display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                      ...
                      <h1 style={{ margin: 0, maxWidth: 760, fontSize: viewportWidth >= 1280 ? 'clamp(56px, 5.2vw, 84px)' : viewportWidth >= 900 ? 'clamp(42px, 6vw, 62px)' : 'clamp(34px, 9vw, 48px)', lineHeight: 0.96, letterSpacing: '-0.05em', ...DS.gradTitle }}>Un cockpit conversationnel plus fin pour lire votre moment.</h1>
                      <p style={{ margin: '20px 0 0', maxWidth: 640, fontSize: viewportWidth >= 900 ? 18 : 16, color: DS.tx2, lineHeight: 1.82, fontFamily: DS.fontBody }}>HexAstra commence par vous écouter, clarifie votre question, puis approfondit si nécessaire avec vos données de naissance. L’expérience reste simple côté utilisateur, avec un rendu plus net, plus rapide et plus élégant à chaque étape.</p>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: viewportWidth >= 700 ? 'repeat(3, minmax(0, 1fr))' : '1fr', gap: 10, marginTop: 28 }}>
                        {[
                          ...
                          ['Rendu premium', 'Un chat plus fin, mieux hiérarchisé et prêt pour une vraie sensation produit.'],
                        ].map(([title, desc]) => (
          ...
          <div style={{ position: 'sticky', bottom: 0, zIndex: 20, padding: viewportWidth >= 1024 ? '10px 28px 22px' : '10px 14px 16px', background: 'linear-gradient(180deg, rgba(10,7,5,0.00), rgba(10,7,5,0.62) 24%, rgba(10,7,5,0.96) 100%)', backdropFilter: 'blur(18px)', WebkitBackdropFilter: 'blur(18px)' }}>
            <div style={{ maxWidth: 980, margin: '0 auto' }}>
              ...
              <ComposerBox>
                {(focused) => (
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, background: 'linear-gradient(180deg, rgba(255,255,255,0.06), rgba(255,255,255,0.028))', border: `1px solid ${focused ? 'rgba(212,165,116,0.34)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 28, padding: viewportWidth >= 900 ? '14px 16px' : '12px 12px', transition: 'border-color 0.25s ease, box-shadow 0.25s ease, transform 0.25s ease', boxShadow: focused ? '0 0 0 2px rgba(212,165,116,0.08), 0 30px 90px rgba(0,0,0,0.38), 0 0 44px rgba(212,165,116,0.10)' : '0 20px 64px rgba(0,0,0,0.24)', backdropFilter: 'blur(22px)', WebkitBackdropFilter: 'blur(22px)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', margin: '10px 0 0' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 12px', borderRadius: 999, border: `1px solid ${DS.border}`, background: 'rgba(255,255,255,0.025)', color: 'rgba(203,185,164,0.62)', fontSize: 11, fontFamily: DS.fontBody }}>
                  <span style={{ color: DS.amber }}>↵</span> Entrée pour envoyer · Maj + Entrée pour revenir à la ligne
                </div>
              </div>
