...
// ② MODALE AVEC CHAMP VILLE (plus de lat/lon)
function BirthModal({ onSubmit, onClose }: { onSubmit: (d: any) => void; onClose: () => void }) {
  ...
  const submit = () => {
    ...
    onSubmit({ birthDateISO: `${date}T${birthTime}:00Z`, lat, lon, city: city.trim() })
  }
        ...
        <div style={m.fields}>
          ...
          <div style={m.field}>
            <div style={m.labelRow}>
              ...
              <label style={m.checkRow}>
                <input
                  ...
                  style={{ accentColor: 'var(--emerald)', marginRight: '5px' }}
                />
            ...
            <input
              ...
              style={{ ...m.input, opacity: unknownTime ? 0.35 : 1 }}
              disabled={unknownTime}
            />
          </div>

          {/* Ville — champ simple, plus de lat/lon */}
          <div style={m.field}>
            <label style={m.label}>Ville de naissance</label>
            <input
              type="text"
              placeholder="Ex : Paris, Lyon, Bordeaux, Casablanca…"
              value={city}
