...
export default function HomePage() {
  ...
  const heroQuery = useMemo(() => heroInput.trim(), [heroInput])
  const isTyping = heroQuery.length > 0
  const isLogoLive = isFocused || isTyping
  ...
  const goToChat = () => {
    ...
    router.push(`/chat?q=${encodeURIComponent(heroQuery)}`)
  }
      ...
      <section className="hx-home-hero">
        ...
        <header className="hx-home-header">
          ...
          <Link href="/" className="hx-home-brand" aria-label="HexAstra Coach">
            <div className={`hx-home-brand-badge ${isLogoLive ? 'is-live' : ''}`}>
              <span className="hx-home-brand-halo" />
              <HexastraLogo
                size={25}
                variant="navbar"
                priority
                className={isLogoLive ? 'is-luminous' : ''}
              />
            </div>
        ...
        <div className="hx-home-hero-inner">
          <div className="hx-home-hero-grid">
            <div className="hx-home-hero-left">
              ...
              <form
                className={`hx-home-search-shell ${isLogoLive ? 'is-live' : ''}`}
                onSubmit={handleSubmit}
              >
                <div className="hx-home-search-glyph">✦</div>
                <input
                  type="text"
                  value={heroInput}
                  ...
                  className="hx-home-search-input"
                  placeholder={rotatingQuestions[placeholderIndex]}
                  aria-label={t('home.heroInputLabel')}
                />
                <button
                  type="submit"
                  className={`hx-home-search-button ${heroQuery ? 'is-ready' : ''}`}
                >
                  {t('home.heroAnalyze')}
                </button>
              </form>
