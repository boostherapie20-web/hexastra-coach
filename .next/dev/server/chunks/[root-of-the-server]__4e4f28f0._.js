...
function buildDisplayName(birthData) {
...
function sanitizeMessages(messages) {
    ...
    return messages.filter((m)=>!!m && typeof m === 'object').map((m)=>({
        ...
        })).filter((m)=>m.content.length > 0).slice(-16);
}
function buildNeedsBirthData(reply, birthData) {
    if (birthData?.date) return false;
    const text = reply.toLowerCase();
    return text.includes('date de naissance') || text.includes('heure de naissance') || text.includes('lieu de naissance') || text.includes('ville de naissance') || text.includes('birth date') || text.includes('birth time') || text.includes('birth place');
}
...
async function fetchEphemerisData(birthData, req) {
    ...
    try {
        const [fusionRes, hdRes, numerologyRes, kuaRes, enneagramRes] = await Promise.allSettled([
            fetch(internalUrl('/api/fusion', req), opts({
                dateISO,
                lat,
                lon
            })),
            fetch(internalUrl('/api/hd', req), opts({
                dateISO
            })),
            fetch(internalUrl('/api/numerology', req), opts({
                dateISO,
                name
            })),
            fetch(internalUrl('/api/kua', req), opts({
                dateISO,
                gender
            })),
            fetch(internalUrl('/api/enneagram', req), opts({
                dateISO,
                name
            }))
        ]);
        ...
        if (fusionRes.status === 'fulfilled' && fusionRes.value.ok) {
            ...
            const aspects = (d?.tropical?.aspects || []).slice(0, 6).map((a)=>`${a.p1}-${a.p2} ${a.type}`).join(', ');
            const ascDelta = d?.fusionMeta?.ascDelta != null ? Number(d.fusionMeta.ascDelta).toFixed(1) : '?';
            parts.push(`Astrologie tropicale : ${[
                sun,
                moon,
                asc
        ...
        if (hdRes.status === 'fulfilled' && hdRes.value.ok) {
            ...
            const channels = (d?.definedChannels || []).map((c)=>c.channel).slice(0, 6).join(', ');
...
async function chatCompletionsFallback(apiKey, messages, threadId) {
    try {
...
async function POST(req) {
    ...
    const birthData = body?.birthData && typeof body.birthData === 'object' ? body.birthData : null;
    const threadId = body?.threadId || body?.conversationId || crypto.randomUUID();
    const inputMessages = [];
    if (birthData?.date) {
        const ephemerisContext = await fetchEphemerisData(birthData, req);
        inputMessages.push({
            role: 'user',
            content: `[Données de naissance] Prénom : ${buildDisplayName(birthData)} · ` + `Date : ${birthData.date || 'non précisée'} · ` + `Heure : ${birthData.time || 'inconnue'} · ` + `Lieu : ${birthData.place || `lat ${birthData.lat}, lon ${birthData.lon}`}` + ephemerisContext
        });
        inputMessages.push({
            role: 'assistant',
            content: 'Données de naissance et calculs précis enregistrés. Je génère la lecture personnalisée.'
        });
    }
    if (mode === 'praticien') {
        inputMessages.push({
