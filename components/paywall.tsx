...
interface PaywallProps {
  userPlan: Plan
  onUpgrade?: () => void
  // Pour le bloc inline dans le chat
  variant?: 'inline' | 'full'
}

// ─── Bloc paywall inline (dans une bulle du chat) ─────────────────────────
    ...
    <div style={p.wrap}>
      ...
      <div style={p.gate}>
        <div style={p.gateIconRow}>
          ...
          <div>
            <div style={p.gateTitle}>Lecture complète disponible</div>
        ...
        <div style={p.gateFeatures}>
          {[
            { icon: '📄', label: "L'analyse détaillée" },
            { icon: '✨', label: 'Les conseils personnalisés' },
            { icon: '🎧', label: 'La version audio' },
            { icon: '📥', label: 'Le PDF téléchargeable' },
          ].map((f, i) => (
...
// ─── Limite quotidienne atteinte (1 lecture/jour) ─────────────────────────
...
// ─── Limite de sauvegarde atteinte (3 lectures max) ───────────────────────
export function StorageLimitReached({ onUpgrade }: { onUpgrade?: () => void }) {
  ...
  return (
    <div style={d.wrap}>
      <div style={d.icon}>📚</div>
      <div style={d.title}>Bibliothèque pleine</div>
      <p style={d.desc}>
...
// ─── Banner upgrade subtil (en bas du chat) ───────────────────────────────
      ...
      <div style={b.left}>
        ...
        <span style={b.txt}>
          {isLow
            ...
            : `${remaining} lecture${remaining > 1 ? 's' : ''} gratuite${remaining > 1 ? 's' : ''} restante${remaining > 1 ? 's' : ''} aujourd'hui`
          }
        </span>
      </div>
  ...
  const plans = [
    {
      ...
      price: '0',
      per: '',
      desc: 'Pour découvrir HexAstra sans engagement.',
      features: [
        { text: '1 lecture gratuite par jour', ok: true },
        { text: 'Analyse courte', ok: true },
        { text: 'Texte uniquement', ok: true },
        { text: 'Accès au chat HexAstra', ok: true },
        { text: 'Sauvegarde limitée (3 lectures)', ok: true },
        { text: 'PDF téléchargeable', ok: false },
        { text: 'Version audio', ok: false },
        { text: 'Analyses avancées', ok: false },
      ],
    ...
    },
    {
      ...
      features: [
        { text: 'Lectures illimitées', ok: true },
        { text: 'Analyses complètes et détaillées', ok: true },
        { text: 'PDF téléchargeables', ok: true },
        { text: 'Audio personnalisé', ok: true },
        { text: 'Historique complet', ok: true },
        { text: 'Thèmes avancés', ok: true },
    ...
    },
    {
      ...
      features: [
        ...
        { text: 'Génération prioritaire', ok: true },
        { text: 'Support dédié', ok: true },
      ],
      cta: 'Démarrer Praticien →',
    ...
    <div style={pr.root}>
      ...
      <nav style={pr.nav}>
        <a href="/" style={pr.navLogo}>
          <span style={pr.navLogoTxt}>Hex<span style={{ color: 'var(--emerald)' }}>Astra</span></span>
        </a>
      ...
      <div style={pr.container}>
        <div style={pr.header}>
          ...
          <p style={pr.headerDesc}>Commencez gratuitement. Évoluez selon vos besoins. Sans engagement.</p>
        </div>

        <div style={pr.grid}>
          {plans.map((plan, i) => (
            <div key={i} style={{ ...pr.card, ...(plan.featured ? pr.cardFeatured : {}) }}>
              {plan.featured && <div style={pr.featuredBadge}>⭐ Le plus populaire</div>}

              <div style={pr.cardTop}>
                <div style={{ ...pr.planTag, color: plan.featured ? 'var(--emerald)' : 'var(--tx3)' }}>{plan.tag}</div>
                <div style={pr.planName}>{plan.name}</div>
                <div style={pr.planPrice}>
                  <span style={pr.planAmt}>{plan.price}</span>
                  {plan.price !== '0' && <span style={pr.planCur}>€</span>}
                  {plan.price === '0' && <span style={pr.planCur}>€</span>}
              ...
              <ul style={pr.features}>
                {plan.features.map((f, j) => (
                  <li key={j} style={f.ok ? pr.feature : pr.featureLocked}>
                    <span style={f.ok ? pr.check : pr.cross}>{f.ok ? '✓' : '✕'}</span>
                    {f.text}
                  </li>
                ))}
              </ul>
        ...
        <div style={pr.faq}>
          <div style={pr.faqTag}>// Questions fréquentes</div>
          {[
            { q: 'Puis-je annuler à tout moment ?', a: 'Oui, sans engagement ni frais. Votre accès reste actif jusqu'à la fin de la période payée.' },
            { q: 'Que contient la lecture gratuite ?', a: 'Une analyse courte en texte uniquement, basée sur vos données de naissance. Pas de PDF ni d'audio.' },
            { q: 'La formule Praticien inclut-elle plusieurs comptes ?', a: 'Non, il s'agit d'un compte individuel avec des droits d'usage pour vos clients.' },
          ].map((item, i) => (
...
const pr: Record<string, React.CSSProperties> = {
  ...
  headerTag: { fontFamily: 'var(--f-mono)', fontSize: '9.5px', letterSpacing: '0.18em', color: 'var(--emerald)', textTransform: 'uppercase' },
  ...
  planTag: { fontFamily: 'var(--f-mono)', fontSize: '9px', letterSpacing: '0.18em', textTransform: 'uppercase' },
  ...
  planCur: { fontFamily: 'var(--f-mono)', fontSize: '18px', color: 'var(--tx2)' },
  ...
  faqTag: { fontFamily: 'var(--f-mono)', fontSize: '9.5px', letterSpacing: '0.18em', color: 'var(--emerald)', textTransform: 'uppercase', padding: '28px 0 20px' },
